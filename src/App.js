    ...
    return (
        <header style={{
            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h1 style={{ margin: 0, fontSize: '24px', fontWeight: '800', letterSpacing: '-0.3px' }}>🥗 InstaMeal</h1>
...
function RestaurantList({ restaurants, loading, userAllergens, tempRestrictions, onOpenSettings }) {
    ...
    if (!restaurants || restaurants.length === 0) return (
        <div style={{ textAlign: 'center', padding: '40px', background: 'white', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.07)', border: '1px solid #d1fae5' }}>
            ...
            <p style={{ color: '#6b7280', margin: 0 }}>Click "Load Korpa Restaurants" or "Find Nearby Restaurants" to get started.</p>
        </div>
    );
...
function Dashboard() {
    ...
    async function loadKorpa() {
        ...
        try {
            ...
            if (userLocation && data.length > 0) {
                setRestaurants(sortByDistance(data, userLocation.lat, userLocation.lon));
            } else {
                setRestaurants(data);
            }
    ...
    async function findNearby() {
        ...
        navigator.geolocation.getCurrentPosition(async pos => {
            ...
            if (restaurants.length > 0) setRestaurants(sortByDistance(restaurants, lat, lon));
            try {
